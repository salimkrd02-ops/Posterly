...
export default function ResultsPage() {
  ...
  if (!activeEvent) return <DashboardShell active="PROGRAM POSTERS:Results"><p>Please create or select an event first.</p></DashboardShell>;

  return (
    <DashboardShell active="PROGRAM POSTERS:Results">
      <div className="mx-auto max-w-[1640px]">
        <div className="flex flex-col items-start justify-between gap-6 xl:flex-row">
          <header>
            <h1 className="text-2xl font-black">Manage Results</h1>
            <p className="mt-3 text-xl text-slate-600">
              View, create, edit, and generate posters for event:{' '}
        ...
        <section className="mt-9 rounded-xl border border-slate-300 bg-white p-6 shadow-sm shadow-slate-300/60">
          <div className="grid gap-4 lg:grid-cols-[1fr_170px_170px]">
            <input placeholder="Search your results..." className="h-10 rounded-md border border-slate-300 bg-white px-4 text-sm outline-none focus:border-[#352bff] focus:ring-2 focus:ring-[#352bff]/15" />
            <select className="h-10 rounded-md border border-slate-300 bg-white px-4 text-sm text-slate-600"><option>All Status</option></select>
            <select className="h-10 rounded-md border border-slate-300 bg-white px-4 text-sm text-slate-600"><option>Sort by Date</option></select>
          </div>
        </section>
        ...
        <section className="flex min-h-[430px] items-center justify-center text-center">
          <div>
            ...
            <h2 className="mt-5 text-xl font-black">No Results Yet</h2>
            <p className="mt-4 text-base text-slate-600">
              You haven&apos;t created any result posters yet. Get started by creating your first one!
            </p>
            <button className="mt-7 rounded-lg bg-[#4338ff] px-5 py-3 text-sm font-bold text-white shadow-sm shadow-indigo-200 hover:bg-[#372ee6]">
              Create Your First Result Poster
            </button>
          </div>
        </section>
      </div>
